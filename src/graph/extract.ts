...
function walk(node: Parser.SyntaxNode, ctx: WalkCtx, out: NodeV1[], edges: RawEdge[], minted: Set<string>): void {
  ...
  if (desc) {
    ...
    // Go method that's its receiver type and for a Rust impl method that's its impl
    // type (neither is nested under an emitted symbol, so ctx.enclosingClass wouldn't
    // see it); for every other method it's simply what the nearest ancestor class
    ...
    const owner: string | undefined =
      desc.kind === "method"
        ? isGoMethod
          ? (goReceiverType(node) ?? undefined)
          : ctx.lang === "rust"
            ? (rustImplType(node) ?? undefined)
            : (ctx.enclosingClass ?? undefined)
        : undefined;
    out.push({
      id,
      name: desc.name,
